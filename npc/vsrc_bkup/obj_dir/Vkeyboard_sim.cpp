...
//============================================================
// Events and timing
bool Vkeyboard_sim::eventsPending() { return false; }

uint64_t Vkeyboard_sim::nextTimeSlot() {
...
std::unique_ptr<VerilatedTraceConfig> Vkeyboard_sim::traceConfig() const {
...
void Vkeyboard_sim___024root__trace_init_top(Vkeyboard_sim___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vkeyboard_sim___024root__trace_init_top(vlSelf, tracep);
...
VL_ATTR_COLD void Vkeyboard_sim::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vkeyboard_sim::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vkeyboard_sim___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
